...
func dead():
	...
	var female = get_parent().get_parent().get_node("EnemyHolder/iceman9/iceman")
	female.hide()
	var female2 = get_parent().get_parent().get_node("EnemyHolder/iceman9/RayCast2D9")
	female2.hide()
...
#
			dead()
			var dead = get_parent().get_parent().get_node("EnemyHolder/iceman9/Particles2D2")
			dead.show()
			$Timer.start()

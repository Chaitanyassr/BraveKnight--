...
func dead():
	...
	var female = get_parent().get_parent().get_node("EnemyHolder/iceman/iceman")
	female.hide()
	var female2 = get_parent().get_parent().get_node("EnemyHolder/iceman/RayCast2D2")
	female2.hide()
...
#	get_tree().reload_current_scene()
	...
	if Input.is_action_pressed("ui_down") and $RayCast2D2.is_colliding() and is_dead == false:
#
			dead()
			var dead = get_parent().get_parent().get_node("EnemyHolder/iceman/Particles2D2")
			dead.show()
			$Timer.start()

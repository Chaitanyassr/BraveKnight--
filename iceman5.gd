...
func dead():
	...
	var female = get_parent().get_parent().get_node("EnemyHolder/iceman5/iceman")
	female.hide()
	var female2 = get_parent().get_parent().get_node("EnemyHolder/iceman5/RayCast2D3")
	female2.hide()
...
#
			dead()
			var dead = get_parent().get_parent().get_node("EnemyHolder/iceman5/Particles2D2")
			dead.show()
			$Timer.start()

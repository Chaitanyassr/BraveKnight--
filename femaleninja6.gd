...
func dead():
	...
	var female = get_parent().get_parent().get_node("EnemyHolder/femaleninja6/femaleninja")
	female.hide()
...
func _physics_process(delta): 
 ...
 if is_on_wall() or $RayCast2D.is_colliding() == false:
	...
	if Input.is_action_pressed("ui_down") and $RayCast2D3.is_colliding() and is_dead == false:
			...
			dead()
			var dead = get_parent().get_parent().get_node("EnemyHolder/femaleninja6/Particles2D3")
			dead.show()
			$Timer.start()

...
func dead():
	...
	var female = get_parent().get_parent().get_node("EnemyHolder/iceman3/iceman")
	female.hide()
	var female2 = get_parent().get_parent().get_node("EnemyHolder/iceman3/RayCast2D3")
	female2.hide()
...
#
			dead()
			var dead = get_parent().get_parent().get_node("EnemyHolder/iceman3/Particles2D2")
			dead.show()
			$Timer.start()
